...
resource "microsoft365wp_cross_tenant_access_policy_configuration_partner" "peers" {
  ...
  # Inbound Trust
  inbound_trust = lookup(each.value.inbound, "trust", null) != null ? {
    is_mfa_accepted                          = lookup(each.value.inbound.trust, "mfa_accepted", null)
    is_compliant_device_accepted             = lookup(each.value.inbound.trust, "compliant_device_accepted", null)
    is_hybrid_azure_ad_joined_device_accepted = lookup(each.value.inbound.trust, "hybrid_azure_ad_joined_device_accepted", null)
  } : null

  # B2B Collaboration Inbound
  b2b_collaboration_inbound = lookup(each.value.inbound, "b2b_collaboration", null) != null ? {
    users_and_groups = {
  ...
  # B2B Collaboration Outbound
  b2b_collaboration_outbound = lookup(each.value.outbound, "b2b_collaboration", null) != null ? {
    users_and_groups = {
